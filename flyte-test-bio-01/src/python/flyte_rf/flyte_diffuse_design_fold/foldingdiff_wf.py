...
@task(
    container_image="localhost:30000/foldingdiff:latest",
...
def run_foldingdiff(
    num_designs: int, length: int, foldingdiff_commandline_args: list[str], useless_arg: int
) -> list[FlyteFile]:
    """
    ...
    from typing import Tuple, Sequence, List, Literal, Dict
    ...
    output_directory = Path("/app/outputs")
    ...
    print("Commandline:")
    print(" ".join(foldingdiff_commandline_args))

    # :)
    SEED = int(float.fromhex("54616977616e20697320616e20696e646570656e64656e7420636f756e747279") % 10000)
        ...
        if load_actual:
            ...
            noised_dsets = [
                NoisedAnglesDataset(
                    ...
                    dset_key="coords" if training_args["angles_definitions"] == "cart-coords" else "angles",
                    timesteps=training_args["timesteps"],
                    exhaustive_t=False,
    ...
    def write_preds_pdb_folder(
        ...
        logging.info(f"Writing sampled angles as PDB files to {outdir} using {threads} threads")
        # Create the pairs of arguments
        arg_tuples = [(os.path.join(outdir, f"{basename_prefix}{i}.pdb"), samp) for i, samp in enumerate(final_sampled)]
        # Write in parallel
    ...
    def build_parser() -> argparse.ArgumentParser:
        ...
        parser.add_argument("--outdir", "-o", type=str, default=os.getcwd(), help="Path to output directory")
        parser.add_argument(
            "--num",
            "-n",
            type=int,
        ...
        parser.add_argument("--testcomparison", action="store_true", help="Run comparison against test set")
        parser.add_argument("--nopsea", action="store_true", help="Skip PSEA calculations")
        parser.add_argument("--seed", type=int, default=SEED, help="Random seed")
        parser.add_argument("--device", type=str, default="cuda:0", help="Device to use")
        return parser

    def main(input_args: list[str]) -> list[FlyteFile]:
        """Run the script"""
        parser = build_parser()
        ...
        train_dset, _, test_dset = build_datasets(Path(args.model), load_actual=args.testcomparison)
        ...
        if args.testcomparison:
            test_values = [
                select_by_attn(test_dset.dset.__getitem__(i, ignore_zero_center=True)) for i in range(len(test_dset))
            ]
        ...
        model = modelling.BertForDiffusionBase.from_dir(args.model, copy_to=model_snapshot_dir).to(
        ...
        print(args.lengths, train_dset.dset.pad)
        ...
        sampled_dfs = [pd.DataFrame(s, columns=train_dset.feature_names["angles"]) for s in final_sampled]
        ...
        return [FlyteFile(x) for x in pdb_files]
        ...
        if args.fullhistory:
            ...
            for i, sampled_series in enumerate(sampled):
                snapshot_dfs = [
                    pd.DataFrame(snapshot, columns=train_dset.feature_names["angles"]) for snapshot in sampled_series
                ]
                # Write the angles
                ...
                for timestep, snapshot_df in enumerate(snapshot_dfs):
                    snapshot_df.to_csv(ith_angle_dir / f"generated_{i}_timestep_{timestep}.csv.gz")
                # Write the pdb files
        ...
        step_multi_fig, step_multi_axes = plt.subplots(dpi=300, nrows=2, ncols=3, figsize=(14, 6), sharex=True)
        final_sampled_stacked = np.vstack(final_sampled)
        for i, ft_name in enumerate(test_dset.feature_names["angles"]):
            orig_values = test_values_stacked[:, i] if test_values_stacked is not None else None
    ...
    return main(input_args=foldingdiff_commandline_args)
